...
function go_get_sharing_acls(shared_endpoint) {
    var resource = '/endpoint/' + encodeURIComponent(shared_endpoint) + '/access_list';
...
function go_add_sharing_acl(shared_endpoint, user, path, permissions) {
    ...
    var resource = '/endpoint/' + encodeURIComponent(shared_endpoint) + "/access";
...
function go_list_endpoints(fields, parameters) {
    optionsArray = [];

    if (parameters.offset) {
        optionsArray.push('offset=' + parameters.offset);
    }
    if (parameters.limit) {
...
function go_auto_activate(endpoint, message_callback) {
    var resource = '/endpoint/' + encodeURIComponent(endpoint) + '/autoactivate?if_expires_in=600';
...
function go_tapi(method, resource, data, error_prefix, callback) {
    ...
    $.ajax({
        ...
        error: function(xhr, status, error) {
            if (debug_alerts) {
                alert('ERROR ' + error_prefix + ' (' + status + ', ' + error + ')');
            }
        },
