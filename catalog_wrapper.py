...
## Catalog wrapper wraps the features of the Globus Catalog API, and includes an interface to interact with Globus Transfer 
    ...
    def transfer_members(self, catalog_id, dataset_id, member_id=None, local_path=None):
        selector_list = []
        if member_id:
            selector_list += [("id", Op.EQUAL, member_id)]

        _,self.members = self.catalogClient.get_members(catalog_id,dataset_id, selector_list=selector_list)
        print self.members
        ...
        for bundle in self.transfer_queue:
            ...
            for transfer in bundle:
                self.debug_list.append('== Starting transfer of %s || [%s] %s to [%s] %s'%(transfer['data_type'],transfer['endpoint'],
                                                                          transfer['location'],self.destination_endpoint, 
                                                                          self.destination_path))
                if transfer['data_type'] == 'file':
                    self.debug_list.append('== Transferring File [%s] %s => [%s] %s'%(transfer['endpoint'], transfer['location'],self.destination_endpoint, self.destination_path))
                    #transfer_object.add_item(trans['location'], self.destination_path)
                elif transfer['data_type'] == 'directory':
                    self.debug_list.append('== Transferring Directory || Adding Recursion [%s] %s => [%s] %s'%(transfer['endpoint'], transfer['location'], self.destination_endpoint, self.destination_path))
    ...
    def extract_transfer_details(self, member_list):
        ...
        for member in member_list:
            ...
            if match != []:
                ...
                tmpDict['data_type'] = member['data_type']
